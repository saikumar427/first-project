...
<xsl:variable name="height-y">
   	<xsl:value-of select="(10 + count(info) * 21) * 2 "/>
 </xsl:variable>

<svg  width="{$width-svg}" height="{$height-y}">
...
<text style="font-size:14; text-anchor:start" x="38">
      <xsl:attribute name="y">
        <xsl:value-of select="$y-legend-offset"/>
      </xsl:attribute>
      <xsl:value-of select="Name"/>

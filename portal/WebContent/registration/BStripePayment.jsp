...
public HashMap<String,String> processStripePayment(CreditCardModel ccm,HttpServletRequest request,String token){
	 String cardnumber=null,expmonth=null,expyear=null,cvvcode=null,name=null,address1=null,address2=null,city=null,zipcode=null,state=null,country=null;
	 String fname="",lname="";
     ...
     resultMap=PrepareResultMap(result);
      ...
      String message="";
      System.out.println("CardException occured : "+e.getMessage()+e.getCode());
      ...
      System.out.println("InvalidRequestException occured : "+e.getMessage()); 
      ...
      // (maybe you changed API keys recently)
    	 System.out.println("AuthenticationException occured : "+e.getMessage());
     }catch (APIConnectionException e) {
       // Network communication with Stripe failed
    	 System.out.println("APIConnectionException occured : "+e.getMessage());
     }catch (StripeException e) {
       // Display a very generic error to the user, and maybe send
      // yourself an email
    	 System.out.println("StripeException occured : "+e.getMessage());
     }catch (Exception e) {
  ...
  if(!"".equals(resultMap.get("id")) && resultMap.get("id")!=null && resultMap.get("livemode").equalsIgnoreCase(accountstatus)){
  ...
  if(!"".equals(resultMap.get("cardnumber")) && resultMap.get("cardnumber")!=null)
	  ...
	  cardParams.put("number",creditcardnumber);
    	...
    	System.out.println("Exception occured while getting cardnumber from chargeParams"+e.getMessage());
    }
  }
  chargeParams.put("card",cardParams);
...
public HashMap<String,String> PrepareResultMap(Charge result){
	HashMap<String,String> resultMap=new HashMap<String,String>(); 
     ...
     resultMap.put("currency",result.getCurrency());
     resultMap.put("customer",result.getCustomer());
     resultMap.put("description",result.getDescription());
     resultMap.put("id",result.getId());
     resultMap.put("created",result.getCreated()+"");
     resultMap.put("capturestatus",result.getCaptured()+"");
     resultMap.put("livemode",result.getLivemode()+"");
     resultMap.put("paidstatus",result.getPaid()+"");
     resultMap.put("refundstatus",result.getRefunded()+"");
     resultMap.put("refundamount",result.getAmountRefunded()+"");
     }catch(Exception e){
...
public String getManagerAPIKey(String eventid){
	String apikey="";
	try{
		apikey=DbUtil.getVal("select attrib_1 from eventbee_manager_sellticket_settings where eventid=cast(? as bigint) and vendor_type=? ",new String[]{eventid,"stripe"});
...
public void insertStripePaymentData(String eventid,String tid,Map<String,Object> inputdatamap,String striperesponse,HashMap<String,String> resultMap,String apikey){
	...
	DbUtil.executeUpdateQuery(query, new String[]{eventid,tid,inputdatamap.toString(),striperesponse,apikey,DateUtil.getCurrDBFormatDate(),resultMap.get("status")});
		...
		System.out.println("Exception occured in insertStripePaymentData : "+eventid+" "+tid+" "+e.getMessage());
	}
}
...
public void insertCardTransactionData(String eventid,String tid,Map<String,Object> inputdatamap,HashMap<String,String> resultMap){
    ...
    try{
    	...
    	DbUtil.executeUpdateQuery(resquery, new String[]{cardtranid,"STRIPE - DIRECT",DateUtil.getCurrDBFormatDate(),"EVENT_REGISTRATION",
    			tid,resultMap.get("cardtype"),resultMap.get("expmonth"),resultMap.get("expyear"),CreditCardNumber,amount,
    			resultMap.get("status"),"AD",resultMap.get("id"),resultMap.get("status")});
    	
    }catch(Exception e){
    	System.out.println("Exception occured in insertCardTransactionData : "+eventid+" "+tid+" "+e.getMessage());
    }
}

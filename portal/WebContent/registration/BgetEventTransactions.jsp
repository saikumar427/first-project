...
<%	//Author: Venkat Reddy
	//Version: 0.1
	//File: getEventTransactions.jsp 
	//Created: 11/08/2014 
	//Modified: 11/08/2014 by venkat reddy*/
%>
...
if("".equals(edate)){
	query="select email,discountcode,ordernumber,current_discount,userid,current_amount,tid,to_char(transaction_date, 'Mon DD, YYYY HH:MI AM') as transaction_date,fname ||' '|| lname as name from event_reg_transactions  where eventid=? and upper(paymentstatus) in ('COMPLETED', 'PENDING') order by transaction_date desc" ;
	 statusObj=dbManger.executeSelectQuery(query, new String[]{eid});
}
else{
	query="select email,discountcode,ordernumber,current_discount,userid,tid,current_amount,to_char(transaction_date, 'Mon DD, YYYY HH:MI AM') as transaction_date,fname ||' '|| lname as name from event_reg_transactions   where eventid=? and eventdate=? and upper(paymentstatus) in ('COMPLETED', 'PENDING') order by transaction_date desc" ;
	 statusObj=dbManger.executeSelectQuery(query, new String[]{eid,edate});
	}

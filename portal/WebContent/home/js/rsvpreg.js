...
function successFunc(response){
 ...
 document.getElementById('rsvpreg').innerHTML=response.responseText;
		...
		if(sure == '1' && ntsure == '0' && rec == 'null' && na == 'N'){
			showrsvpLoadingImage(props.loading);
...
for(var i=0;i<=len;i++){
		...
		if(i=='0')
			optn.text = props.rec_sel_count_lbl;
		else
			optn.text = i;
		optn.value = i;
		try {
...
function validateRsvpselectedoptions(){
	...
	if(rsvprecurring == 'Y'){
		if(document.getElementById('rsvp_event_date').value=='--Select Date--'){
			alert(props.rec_sel_dt_time_msg);
	...
	var response=statusjson.responsetype;
		...
		if(status=='Success'){
			if(response=='N'){
				...
				rsvpconformationpage(statusjson.transactionid,emailid,statusjson.ordernumber);
			}
	}
	else
		$('rsvpprofilecontent').innerHTML='completed';
...
function rsvpconformationpage(transactionid,emailid,ordernumber){
...
method: 'get',
parameters:{eventid:evetid,sure:sureselect,notsure:notsureselect,rsvp_event_date:rsvp_event_date,transactionid:transactionid,emailid:emailid,ordernumber:ordernumber},

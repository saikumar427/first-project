...
<%
	...
	if(!("000000".equals(id1)|| "".equals(id1)|| "0".equals(id1))){
	  ...
	  request.setAttribute("tasksubtitle","Registered Events");
	}else{
...
%>
	...
	<% if(!("000000".equals(id1)|| "".equals(id1)|| "0".equals(id1))){ 
      		Vector mylistedevts1=AccountDB.getMemRegEvents(id1,null);
		List mylistedevts=null;
		pageNatingException pageException=null;
		pageNating pageNav=new pageNating();
		try{
			pageNav.setLink(PageUtil.appendLinkWithGroup("/portal/eventdetails/pageregisteredevents.jsp?userid="+id1,(HashMap)request.getAttribute("REQMAP")));
	...
	%>
		...
		<%for(int i=0;mylistedevts!=null && i<mylistedevts.size();i++){
			...
			String refer=ShortUrlPattern.get(GenUtil.getHMvalue(evtMap,"username"))+"/event?eventid="+GenUtil.getHMvalue(evtMap,"eventid");
		%>

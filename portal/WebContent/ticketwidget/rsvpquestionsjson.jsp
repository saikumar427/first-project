	...
	if(customQuestions!=null&&customQuestions.size()>0){
		for(int k=0;k<attributeSet.length;k++){
			...
			if(customQuestions.contains(cb.getAttribId())){
				...
				//else 
				if("textarea".equals(cb.getAttributeType())){
			...
			attributesArray.put(attributesObj);
			ProfileObject.put(pattern,attributesArray);
			}
		}
	}
}
...
ProfileObject.put("Available","NO");
	...
	CProfilePageDisplay profiledisplay=new CProfilePageDisplay();
...
String arribsetid=customattribs.getAttribSetid();
ProfileObject.put("arribsetid",arribsetid);
...
fillCustomQuestions(profilePageLabels,attributeSet,customQuestions,questionsarray,ProfileObject,"buyer_questions");
if(!"0".equals(sure)|| Integer.parseInt(sure)!=0){
	System.out.println("in sure ");
ArrayList customsureQuestions=getQuestionsFortheSelectedOption("101",eventid);
fillCustomQuestions(profilePageLabels,attributeSet,customsureQuestions,surequestionsarray,ProfileObject,"attendee_questions");
}
if(!"0".equals(notsure)|| Integer.parseInt(notsure)!=0){
	System.out.println("in not sure ");
ArrayList customnotsureQuestions=getQuestionsFortheSelectedOption("102",eventid);
fillCustomQuestions(profilePageLabels,attributeSet,customnotsureQuestions,notsurequestionsarray,ProfileObject,"attendee_questions"); 
}

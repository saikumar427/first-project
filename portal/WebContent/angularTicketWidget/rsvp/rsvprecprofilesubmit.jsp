...
void fillTransactionLevelQuestions(CustomAttribute[] attributeSet,ArrayList attribsList,ProfileActionDB profiledbaction,HttpServletRequest req,String attendeeid,String attribsetid,String eventid,String profilekey,String transid,JSONObject buyerJSON){
	System.out.println("fillTransactionLevelQuestions");
	...
	for(int j=0;j<attributeSet.length;j++){
		...
		if(attribsList.contains(cb.getAttribId())){
			...
			//lastname=buyerJSON.getString("q_s_lname_1");
			try{
				shortresponse=buyerJSON.getString("q_p_"+questionid);
				bigresponse=buyerJSON.getString("q_p_"+questionid);
			}catch(Exception e){
				System.out.println(" in rsvprecprofilesubmit.jsp"+e);
			}
...
void fillResponseLevelQuestions(CustomAttribute[] attributeSet,ArrayList attribsList,ProfileActionDB profiledbaction,HttpServletRequest req,String attribsetid,String eventid,String pattern,int count,String transid,String promotiontype,JSONObject buyerJSON){
	...
	for(int i=1;i<=count;i++){
		...
		try{
		firstname=buyerJSON.getString(pattern+"fname_"+i);
		lastname=buyerJSON.getString(pattern+"lname_"+i);
		//phone=buyerJSON.getString(pattern+"phone_"+i);
		email=buyerJSON.getString(pattern+"email_"+i);
		}catch(Exception e){
			System.out.println("Exception in rsvp profile submit"+e);
		}
	...
	for(int j=0;j<attributeSet.length;j++){
		...
		if(attribsList.contains(cb.getAttribId())){
			...
			try{
				shortresponse=buyerJSON.getString(pattern+questionid+"_"+i);
				bigresponse=buyerJSON.getString(pattern+questionid+"_"+i);

		}catch(Exception e){
			System.out.println("Ecxeption in rsvp profile submit "+e);
		}
...
String buyer_info=request.getParameter("buyer_info");
...
String attribsetid=request.getParameter("arribsetid");
System.out.println("attribsetid :: "+attribsetid);
String transid="";
...
try{
	...
	JSONObject buyerJSON = new JSONObject(buyer_info);
	System.out.println("length" +buyerJSON);
	//{"q_s_fname_1":"sai","q_s_lname_1":"kumar","q_s_email_1":"sai.pajworld@gmail.com","q_s_2_1":"q1","q_s_3_1":"t1","q_p_1":"1111111","q_p_4":"zx xz xz zx"}
	
String firstname="",lastname="",email="",phone="";
if(buyerJSON.has("q_p_fname"))
firstname=buyerJSON.getString("q_p_fname");
if(buyerJSON.has("q_p_lname"))
lastname=buyerJSON.getString("q_p_lname");
if(buyerJSON.has("q_p_email"))
email=buyerJSON.getString("q_p_email");
if(buyerJSON.has("q_p_phone"))
phone=buyerJSON.getString("q_p_phone");
rec_event_date=request.getParameter("rsvp_event_date");
if("yes".equals(option)){
	if(!"0".equals(surecount)){
		firstname=buyerJSON.getString("q_s_fname_1");
		lastname=buyerJSON.getString("q_s_lname_1");
		email=buyerJSON.getString("q_s_email_1");
			
	}
	if(!"0".equals(notsurecount)){
		firstname=buyerJSON.getString("q_ns_fname_1");
		lastname=buyerJSON.getString("q_ns_lname_1");
		email=buyerJSON.getString("q_ns_email_1");
...
fillTransactionLevelQuestions(attributeSet,attribsList,profiledbaction,request,attendeeid,attribsetid,eventid,profilekey,transid,buyerJSON);
...
int notsureattend=Integer.parseInt(request.getParameter("notsure"));
    if(sureattend!=0)
	fillResponseLevelQuestions(attributeSet,sureattribsList,profiledbaction,request,attribsetid,eventid,"q_s_",sureattend,transid,promotiontype,buyerJSON);
	if(notsureattend!=0)
	fillResponseLevelQuestions(attributeSet,notsureattribsList,profiledbaction,request,attribsetid,eventid,"q_ns_",notsureattend,transid,promotiontype,buyerJSON);

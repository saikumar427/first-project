...
<%!
	String CLASS_NAME="coupon/logic/CouponEditScreen1.jsp";
...
discounttype=DbUtil.getVal("select discounttype from coupon_master where couponid=? and groupid=?",new String []{couponid,GROUPID});
couponhash.put("disctype",discounttype);
 String limitchecked=null;
if(couponhash!=null)
 limitchecked=(String)couponhash.get("code_limittype");
if(limitchecked==null)
limitchecked="0";
discounttype=(String) couponhash.get("disctype");
...
// discounttype=(String)couponhash.get("discounttype");
%>
<script>
...
//alert("discperc"+discperc)
 ...
 if(discperc>100){
 alert("Discount Percentage should not be above 100%");
...
<form  action="/discounts/CouponEdit.jsp;jsessionid=<%=session.getId()%>" method="post" name="editdisc"  >
...
<input type="hidden" name="coupontype" value="General"/>
...
<table align="center" width="100%">
	...
	<tr><td class="inputlabel" >Description</td><td  class="inputvalue"><textarea name="desc" rows="5" cols="40" onfocus="this.value=(this.value==' ')?'':this.value"><%=GenUtil.getEncodedXML(getValuefromSess(couponhash,"desc")).trim() %> </textarea></td></tr>
	<tr>
	...
	<%=currency%><input type="text" size="6" name="discount"  value="<%=GenUtil.getEncodedXML(getValuefromSess(couponhash,"discount")).trim() %>"/></td></tr>
	--%>
	<tr><td class="inputlabel" >Discount *</td><td  class="inputvalue">
	...
	<td  class="inputvalue">
		<input type="text" name="codes" size="42" value="<%=GenUtil.getEncodedXML(getValuefromSess(couponhash,"codes")).trim() %>" />
	</td></tr>

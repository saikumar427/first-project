...
public class CRegistrationDBHelper {
	...
	public CRegistrationDBHelper(String type){
		confirmationtype=type;
	}
	public CRegistrationDBHelper(){
	...
	HashMap getAttendeeDetails(String tid){
		HashMap hm=new HashMap();
		DBManager db=new DBManager();
		...
		if(sb.getStatus()){
			...
			for(int k=0;k<sb.getCount();k++){
				...
				if(hm.containsKey(db.getValue(k,"ticketid",""))){
					attndeelist=(ArrayList)hm.get(db.getValue(k,"ticketid",""));
		...
		if(sb.getStatus()){
			for(int i=0;i<sb.getCount();i++){
				...
				if(hm.containsKey(pkey)){
					...
					temp.add(new Entity(attname,db.getValue(i, "response", "")));
					hm.put(pkey, temp);
				}
				else{
...
try{
String  TRANSACTION_DETAILS="select current_discount,current_amount,paymentstatus,original_tax,ordernumber,eventdate,paymenttype,ext_pay_id,bookingdomain  from event_reg_transactions where tid=?";
DBManager db=new DBManager();

...
public class CProfileActionDB {
	...
	public void updateBaseProfile(HashMap baseprofile){
		try{
			...
			String profilebasequery="insert into profile_base_info(eventid,fname,lname,transactionid,phone,email,profilekey,ticketid,tickettype,profileid,created_at,seatcode,profile_setid,seatindex)"
				+" values(CAST(? AS BIGINT),?,?,?,?,?,?,CAST(? AS BIGINT),?,CAST(? AS BIGINT),to_timestamp(?,'YYYY-MM-DD HH24:MI:SS.MS'),?,?::bigint,?)";
			...
			EventbeeLogger.log(EventbeeLogger.LOGGER_MAIN,EventbeeLogger.INFO, "CProfileActionDB.java", "Status Of the inserting updateBaseProfile into profilebaseinfo for the transactionid---->"+baseprofile.get("tid"), ""+s1.getStatus(), null );
		}
		catch(Exception e){
	...
	public void InsertResponseMaster(HashMap<String,String> responseMaterMap){
		try{
			...
			EventbeeLogger.log(EventbeeLogger.LOGGER_MAIN,EventbeeLogger.INFO, "CProfileActionDB.java", "Status Of the inserting Entry into respnseMaster for the transactionid---->"+responseMaterMap.get("tid"), ""+s.getStatus(), null );
		}
		catch(Exception e){
	...
	public void insertResponse(HashMap<String, String> respnseMap){
		try{
			...
			EventbeeLogger.log(EventbeeLogger.LOGGER_MAIN,EventbeeLogger.INFO, "CProfileActionDB.java", "Status Of the inserting Response into insertResponse for the transactionid---->"+respnseMap.get("responseid"), ""+sb1.getStatus(), null );
		}
		catch(Exception e){
	...
	public void inserBuyerInfo(HashMap<String,String> buyerInfo){
		try{
			StatusObj sb=DbUtil.executeUpdateQuery("insert into buyer_base_info(fname,lname,email,phone,profileid,profilekey,transactionid,eventid,created_at) values (?,?,?,?,CAST(? AS INTEGER),?,?,CAST(? AS INTEGER),to_timestamp(?,'YYYY-MM-DD HH24:MI:SS.MS'))",new String[]{(String)buyerInfo.get("fname"),(String)buyerInfo.get("lname"),(String)buyerInfo.get("email"),(String)buyerInfo.get("phone"),(String)buyerInfo.get("profileid"),(String)buyerInfo.get("profilekey"),(String)buyerInfo.get("tid"),(String)buyerInfo.get("eventid"),DateUtil.getCurrDBFormatDate()});
			EventbeeLogger.log(EventbeeLogger.LOGGER_MAIN,EventbeeLogger.INFO, "CProfileActionDB.java", "Status Of the inserting buyerInfo into buyer_base_info for the transactionid---->"+buyerInfo.get("tid"), ""+sb.getStatus(), null );
		}
		catch(Exception e){
		...
		try{
			...
			if(options!=null){
				for(int p=0;p<options.size();p++){
					CAttribOption attribop=(CAttribOption)options.get(p);
	...
	public void InsertAttendeeInfo(HashMap attendeeMap){
		...
		String query="insert into rsvpattendee(firstname,lastname,attendeekey,attendeeid,eventid,phone,email,priattendee,attendeecount,attendingevent) values(?,?,?,CAST(? AS INTEGER),CAST(? AS INTEGER),?,?,?,?,?)";

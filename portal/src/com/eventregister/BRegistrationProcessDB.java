...
public class BRegistrationProcessDB{
	...
	public String  insertRegistrationDb(String tid,String eid){
		...
		System.out.println("(Box office)Status Of the inserting tickets into transaction_tickets for the transactionid---->"+tid+"---"+s.getStatus());
		...
		if(sb1.getStatus() && sb1.getCount()>0)
			System.out.println("(Box office)DUPLICATE-TRANSACTIONID found for tid: "+tid+", No of Duplicates deleted: "+sb1.getCount());
		...
		if("other".equals(paymenttype) || "google".equals(paymenttype) || ("paypal".equals(paymenttype) && !"chained".equals(EventRegData.get("paymentmode"))) || "authorize.net".equals(paymenttype) || "braintree_manager".equals(paymenttype)|| "stripe".equals(paymenttype)){
			...
			if(!"yes".equals(iscollected) && ebee_usd>0){
				...
				if("yes".equals(avail) && !"yes".equals(iscollected)){
					...
					System.out.println("(Box office)RegistrationProcessDB-"+tid+": credits not available");
			}
		}
		if("ebeecredits".equals(paymenttype)){
			String currencyconversion=regdbhelper.getcurrencyconversion(eid);
		...
		if("YES".equals(seatingenabled)){
			BRegConfirmationDBHelper regConfirmdb=new BRegConfirmationDBHelper();
		...
		System.out.println("(Box office)referral_ntscode at sale count:"+referral_ntscode);
		...
		{   referral_ntscode=referral_ntscode.trim();
			...
			System.out.println(" Box office salecount:"+salecount);
	...
	public void deleteDuplicateProfiles(String tid){
		System.out.println("(Box office)Searching DUPLICATE-BASIC-PROFILES for tid: "+tid);
		...
		if(basicsob.getStatus() && basicsob.getCount()>0){
			System.out.println("(Box office)DUPLICATE-BASIC-PROFILES found for tid: "+tid+", No of Duplicates deleted: "+basicsob.getCount());
		}
		...
		if(buyersob.getStatus() && buyersob.getCount()>0){
			System.out.println("(Box office)DUPLICATE-BUYER-PROFILES found for tid: "+tid+", No of Duplicates deleted: "+buyersob.getCount());
		}
	}

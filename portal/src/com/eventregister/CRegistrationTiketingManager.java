...
public class CRegistrationTiketingManager{
...
public	String  createNewTransaction(String eventid,HashMap contextMap){
   ...
   public HashMap<String,String> getEbeeFee(String eid){
    ...
    public JSONObject getAttendeeDetails(String tid){
    	String profileQuery="select fname,lname,phone,email,profilekey,seatcode,ticketid from profile_base_info where transactionid=?";
    	...
    	try{
    	sb=dbm.executeSelectQuery(profileQuery,new String[]{tid});
    	if(sb.getStatus() && sb.getCount()>0){
    	JSONObject eachObj=null;
    	for(int i=0;i<sb.getCount();i++){
    		if(attendeeDetails.has(dbm.getValue(i,"ticketid","")))
    		attendeeArray=attendeeDetails.getJSONArray(dbm.getValue(i,"ticketid",""));
    		else
    		attendeeArray=new JSONArray();
	    	eachObj=new JSONObject();
	    	eachObj.put("response",new JSONObject().
	    			put("fname",dbm.getValue(i,"fname","")).
	    			put("lname",dbm.getValue(i,"lname","")).
	    			put("email",dbm.getValue(i,"email","")).
	    			put("phone",dbm.getValue(i,"phone","")));
	    	eachObj.put("seat_code",dbm.getValue(i,"seatcode",""));
	    	eachObj.put("profile_key",dbm.getValue(i,"profilekey",""));
	    	attendeeArray.put(eachObj);
	    	attendeeDetails.put(dbm.getValue(i,"ticketid",""), attendeeArray);
    	}
    }
    	}catch(Exception e){}
    ...
    public JSONObject getAttendeeCustomDetails(String eid,String tid){
   	String query="select a.attribid, a.shortresponse as response,b.transactionid,c.attribname,c.attribtype,b.profilekey,b.subgroupid "+
   		    	"from custom_questions_response a,custom_questions_response_master b,custom_attribs c "+
   		    	"where a.ref_id=b.ref_id and b.groupid=CAST(? AS BIGINT) and b.attribsetid::int=c.attrib_setid and a.attribid=c.attrib_id"+
   		    	" and a.attribid not in (select attribid from buyer_custom_questions where eventid=CAST(? AS BIGINT)) and a.bigresponse!='' and b.transactionid=? order by a.ref_id";
	...
	JSONObject attendeeDetails = new JSONObject();
    try{
    	sb=dbm.executeSelectQuery(query,new String[]{eid,eid,tid});
    	if(sb.getStatus() && sb.getCount()>0){
	    	JSONObject eachObj=null;
	    	for(int i=0;i<sb.getCount();i++){
	    		if(attendeeDetails.has(dbm.getValue(i,"profilekey","")))
	    		attendeeArray=attendeeDetails.getJSONArray(dbm.getValue(i,"profilekey",""));
	    		else
	    		attendeeArray=new JSONArray();
		    	eachObj=new JSONObject();
		    	eachObj.put(dbm.getValue(i,"attribid",""),dbm.getValue(i,"response",""));
		    	
		    	if(!"checkbox".equals(dbm.getValue(i,"attribtype","")))
		    		eachObj.put(dbm.getValue(i,"attribid",""),dbm.getValue(i,"response",""));
	   		else{
	   			if(dbm.getValue(i,"response","").indexOf(",")>-1){
	   			List responseList = Arrays.asList(dbm.getValue(i,"response","").split(","));
	   			JSONObject tempObj=new JSONObject();
	   			for(int j=0;j<responseList.size();j++)
	   				tempObj.put(responseList.get(j)+"",true);
	   			eachObj.put(dbm.getValue(i,"attribid",""),tempObj);
	   			}else{
	   				JSONObject tempObj=new JSONObject();
	   				eachObj.put(dbm.getValue(i,"attribid",""),tempObj.put(dbm.getValue(i,"response",""),true));
	   			}
	   		}
		    	attendeeArray.put(eachObj);
		    	attendeeDetails.put(dbm.getValue(i,"profilekey",""), attendeeArray);
	    	}
	   }
    }catch(Exception e){}
    	return attendeeDetails;
    ...
    public JSONObject getFinalResonse(String attendekey,JSONObject jsonObj,JSONObject attendeeCustom){
    	if("".equals(attendekey))
    		return jsonObj;
    	try{
    	JSONObject baseResponse=jsonObj.has("response")?jsonObj.getJSONObject("response"):new JSONObject();
    	JSONArray customResonsesArray=attendeeCustom.has(attendekey)?attendeeCustom.getJSONArray(attendekey):new JSONArray();
    	for(int i=0;i<customResonsesArray.length();i++){
    		JSONObject eachObj=customResonsesArray.getJSONObject(i);
    		Iterator<String> it=eachObj.keys();
    		while(it.hasNext()){
    			String key = it.next()+"";
    		baseResponse.put(key, eachObj.get(key));
    		}
    	}
    ...
    public JSONObject getbuyerCustomResponses(String eid,String tid){
        String query="select a.attribid, a.shortresponse as response,b.transactionid,c.attribname,c.attribtype "+
        	"from custom_questions_response a,custom_questions_response_master b,custom_attribs c "+
        	"where a.ref_id=b.ref_id and b.groupid=CAST(? AS BIGINT) and b.attribsetid::int=c.attrib_setid and a.attribid=c.attrib_id"+
        	" and a.attribid in (select attribid from buyer_custom_questions where eventid=CAST(? AS BIGINT)) and a.bigresponse!='' and b.transactionid=? order by a.ref_id";
        DBManager dbm=new DBManager();
        StatusObj sbj=null;
        sbj=dbm.executeSelectQuery(query,new String[]{eid,eid,tid});
        JSONObject responseObj=new JSONObject();
        try{
        if(sbj.getStatus() && sbj.getCount()>0){
        	for(int i=0;i<sbj.getCount();i++){
        		if(!"checkbox".equals(dbm.getValue(i,"attribtype","")))
        			responseObj.put(dbm.getValue(i,"attribid",""),new JSONObject().put("response",dbm.getValue(i,"response","")));
        		else{
        			if(dbm.getValue(i,"response","").indexOf(",")>-1){
        			List responseList = Arrays.asList(dbm.getValue(i,"response","").split(","));
        			JSONObject tempObj=new JSONObject();
        			for(int j=0;j<responseList.size();j++)
        				tempObj.put(responseList.get(j)+"",true);
        			responseObj.put(dbm.getValue(i,"attribid",""),new JSONObject().put("response",tempObj));
        			}else{
        				JSONObject tempObj=new JSONObject();
        				responseObj.put(dbm.getValue(i,"attribid",""),new JSONObject().put("response",tempObj.put(dbm.getValue(i,"response",""),true)));
        			}
        		}
        	}
        }

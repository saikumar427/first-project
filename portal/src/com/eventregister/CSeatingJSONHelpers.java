...
public class CSeatingJSONHelpers {
	...
	public static JSONObject getSectionSeatingDetails(HashMap hmap){
		...
		try{
			...
			HashMap holdSeatsTid = (HashMap) hmap.get("holdseatstkts");
			
			System.out.println("total hold::"+onholdseats+"::tids::"+holdSeatsTid);
				...
				if("N".equals(seat.get("isseat"))){
					...
					/*	ArrayList ticketnames=new ArrayList();
						...
						*/
						...
						if(onholdseats.contains(seatindex) && !holdSeatsTid.containsKey(seatindex))
							color=color+"_Hold";
						
						if(onholdseats.contains(seatindex) && holdSeatsTid.containsKey(seatindex)){
							temp.put("seatSelected", true);
						    temp.put("selectedTkt", holdSeatsTid.get(seatindex));
						}
						if(groupid.equals("-1"))
							color="NA";	
						temp.put("type", color);

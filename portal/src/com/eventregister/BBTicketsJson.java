...
public class BBTicketsJson {	
	...
	public JSONObject getTicketsJSON(String eid,String edate,String tid,String ticketUrlCode,String discountCode,String source){
		...
		try{
			...
			//System.out.println("wordings"+regFlowWordings);
			...
			ticketsJSON.put("reg_flow_wordings", reg_flow_wordings);
			 ...
			 Double taxExtra=0.00;
			   String taxPercent=DbUtil.getVal("select value from config where name='event.tax.amount' and config_id=(select config_id from eventinfo where eventid=to_number(?,'99999999999999'))",new String[]{eid} );
			...
			ArrayList<BTicketGroup> ticketsArray=ticketInfo.requiredTicketGroups;
			...
			mgrConfigMessages=ticketInfo.getTicketMessage(eid,edate);
			
			Double totalFee=0.0;
			
			for(int i=0;i<ticketsArray.size();i++){
				BTicketGroup eachGroup=(BTicketGroup)ticketsArray.get(i);

...
public class MustByOR implements ConditionalTicketing {
				...
				if(selectedTickets.has(srcTicket)){
					...
					for(int i=0;i<trgArray.length();i++){
						...
						if(selectedTickets.has(trgId)){
							if(!"NOLIMIT".equalsIgnoreCase(trgArray.getJSONObject(i).getString("type")) && selectedTickets.getInt(trgId)<trgArray.getJSONObject(i).getInt("qty")){
								...
								//warningsList.add(" You are not allowed to buy "+srcTicket+" until an  unless  "+trgArray.getJSONObject(i).getInt("qty")+"  "+trgId+"  ticket(s) ");
								return warningsList;
							}
							flag=true;
							...
							/*if(!targetAllowMap.containsKey(srcTicket+"_max")){
								if(trgArray.getJSONObject(i).getInt("qty")==0)//if src qty is none(or =0) //we should not allow ratio
						...
						if("".equals(selTickets.toString())&&selectedTickets.has(trgId)){
							if(trgArray.getJSONObject(i).getInt("qty")!=0)// if src_qty=0 dont put selected qty in warning message, 0 means no limit
							...
							if(trgArray.getJSONObject(i).getInt("qty")!=0)// if src_qty=0 dont put selected qty in warning message
					...
					if(!flag){
						...
						if(trgArray.length() > 1 )
							warningsList.add(" If you want to buy "+srcTicket+",  you have to buy one of the tickets - "+dependantTickets.toString()+".");
						else

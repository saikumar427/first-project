...
import com.customquestions.beans.BCustomAttribResponse;
...
private static final String GET_EVENT_ATTRIB_SET="select b.attrib_setid,a.* from custom_attribs a,"
	...
	+" where a.attrib_setid=b.attrib_setid and b.groupid=CAST(? AS INTEGER) and b.purpose=? order by position ";
private static final String attribIdSelectQuery="select b.attrib_setid,a.* from custom_attribs a,"
	...
	+" where a.attrib_setid=b.attrib_setid and b.groupid=? and b.purpose=? and "
	+" a.attribsetid=c.attribsetid and b.groupid=c.groupid and c.subgroupid=? order by position";
...
public  HashMap<String,ArrayList<BAttribOption>> getAllCustomAttribSetOptions(String attribsetid){
	HashMap<String,ArrayList<BAttribOption>> attribOptionsMap=new HashMap<String,ArrayList<BAttribOption>>();
	DBManager dbmanager=new DBManager();
	StatusObj statobj=null;
	try{
	statobj=dbmanager.executeSelectQuery(GET_ALL_ATTRIB_OPTIONS,new String []{attribsetid});
	...
	for (int k=0;k<count;k++){
		...
		if(attribOptionsMap.containsKey(attribid)){
			ArrayList<BAttribOption> options=(ArrayList<BAttribOption>)attribOptionsMap.get(attribid);
			BAttribOption cop=new BAttribOption();
			...
			ArrayList<BAttribOption> options=new ArrayList<BAttribOption>();
			BAttribOption cop=new BAttribOption();
			cop.setPosition(dbmanager.getValue(k,"position",""));
			cop.setOptionid(dbmanager.getValue(k,"option",""));
			cop.setOptionValue(dbmanager.getValue(k,"option_val",""));
...
public  BCustomAttribSet getCustomAttribSet(String groupid,String purpose){
...
public  BCustomAttribSet getCustomAttribSet(String groupid,String subgroupid,String purpose){

	
	ArrayList<String> params=new ArrayList<String>();
	BCustomAttribSet customAttSet=new BCustomAttribSet();
	DBManager dbmanager=new DBManager();
	...
	int count=statobj.getCount();
	if(statobj.getStatus()&&count>0){
		BCustomAttribute[] attribs=new BCustomAttribute[count];
		String attribSetId=dbmanager.getValue(0,"attrib_setid","");
		HashMap<String,ArrayList<BAttribOption>> optionsOfAttribs=getAllCustomAttribSetOptions(attribSetId);
		customAttSet.setAttribSetid(attribSetId);
			for (int k=0;k<count;k++){
				BCustomAttribute attrib= new BCustomAttribute();
				String attribid=dbmanager.getValue(k,"attrib_id","");
				...
				if(optionsOfAttribs.containsKey(attribid)){
					attrib.setOptions( (ArrayList<BAttribOption>) optionsOfAttribs.get(attribid));
				}// End if()
...
public HashMap<String,ArrayList<String>> getTicketLevelAttributes(String groupid){
	...
	StatusObj sb=db.executeSelectQuery("select subgroupid,a.attribid,a.position as x,b.position as y from subgroupattribs a,custom_attribs b where a.attribsetid=b.attrib_setid and groupid=CAST(? AS INTEGER) and a.attribid=b.attrib_id order by subgroupid,a.position,b.position",new String []{groupid});
	if(sb.getStatus()){
...
public  HashMap getAttribResponseSet(String trnid,String groupid,String subgroupid,String profileid,String purpose)
{
	...
	int count=statobj.getCount();
	//ArrayList
	BCustomAttribResponse custattresp=new BCustomAttribResponse();
	if(statobj.getStatus()&&count>0){
	for (int k=0;k<count;k++){
...
public StatusObj  setAttributeResponsekey(BCustomAttribResponse[] custattrbresp,String trnid,String groupid,String profileid,String subgroupid,String attribsetid)
{
	DBManager dbmanager=new DBManager();
	HashMap hm=new HashMap();
	StatusObj statobj=null;
	...
	BCustomAttribResponse custresp=custattrbresp[i];
...
public  StatusObj insertCustomAttributes(BCustomAttribute [] customattribs,String groupid,String purpose,String setid){
		...
		for(int i=0;i<customattribs.length;i++){
			...
			for(int j=0;j<options.size();j++){
				...
				BAttribOption attopt=(BAttribOption) options.get(j);
				opt.add(id);
				opt.add((j+1)+"");

...
import com.customquestions.beans.CustomAttribResponse;
...
private static final String INSERT_OPTIONS="insert into custom_attrib_options"
	...
	+" values(?,?,?,?,?,now(),now())";
//Rahul
public  HashMap getAllCustomAttribSetOptions(String attribsetid){
	...
	for (int k=0;k<count;k++){
		...
		if(attribOptionsMap.containsKey(attribid)){
			ArrayList options=(ArrayList)attribOptionsMap.get(attribid);
...
public  CustomAttribSet getCustomAttribSet(String groupid,String subgroupid,String purpose){
	...
	HashMap optionsOfAttribs=getAllCustomAttribSetOptions(attribsetid);
	custattset.setAttribSetid(attribsetid);
		...
		if(optionsOfAttribs.containsKey((Object)attribid)){
			attrib.setOptions( (ArrayList) optionsOfAttribs.get(attribid));
		}// End if()
...
StatusObj sb=db.executeSelectQuery("select subgroupid,a.attribid,a.position as x,b.position as y from subgroupattribs a,custom_attribs b where a.attribsetid=b.attrib_setid and a.groupid=CAST(? AS BIGINT) and a.attribid=b.attrib_id order by subgroupid,a.position,b.position",new String []{groupid});
if(sb.getStatus()){
